...
import { SportsManagerStream } from '../typechain/contracts/stream/SportsManagerStream';
...
const deploy = async () => {
  ...
  let factory = await SportsManagerStreamFactory.deploy();
...
const deployTest = async () => {
  let SportsManagerStreamTest = await ethers.getContractFactory('SportsManagerStreamTest');
  let test = await SportsManagerStreamTest.deploy();
  await test.deployed();

  return test;
}

const clone = async (factory: Contract, desc: string, members: SportsManagerStream.MemberStruct[]) => {
...
describe("SportsManager Stream", function() {
  ...
  it('fetch members', async () => {
    await clone(factory, "alice, bob, carol", [{
  ...
  it('2 way equal split', async () => {
    await clone(factory, "alice and bob", [{
      account: alice.address,
      value: 1,
      total: 2,
    }, {
  ...
  it('should fail if sent with send/transfer', async () => {
    ...
    let tx = deployer.sendTransaction({
      to: test.address,
